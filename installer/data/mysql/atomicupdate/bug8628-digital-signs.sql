...
CREATE TABLE signs (
  ...
  PRIMARY KEY (sign_id),
  CONSTRAINT signs_ibfk_1 FOREIGN KEY (saved_sql_id) REFERENCES saved_sql (id) ON DELETE CASCADE
);
...
CREATE TABLE decks (
  ...
  PRIMARY KEY (deck_id),
  CONSTRAINT decks_ibfk_1 FOREIGN KEY (branchcode) REFERENCES branches (branchcode) ON DELETE CASCADE
);
...
CREATE TABLE signs_to_decks (
  ...
  CONSTRAINT signs_to_decks_sdfk_1 FOREIGN KEY (deck_id) REFERENCES decks (deck_id) ON DELETE CASCADE,
  CONSTRAINT signs_to_decks_sdfk_2 FOREIGN KEY (sign_id) REFERENCES signs (sign_id) ON DELETE CASCADE
);
...
INSERT INTO decks ( deck_id, branchcode, name, webapp ) VALUES ( 1, 'CPL', 'Signs for the main library', 1 );
